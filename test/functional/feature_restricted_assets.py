...
class RestrictedAssetsTest(SlimecoinTestFramework):
    ...
    def activate_restricted_assets(self):
        self.log.info("Generating SLME and activating restricted assets...")
        n0 = self.nodes[0]
        n0.generate(432)
        self.sync_all()
        n1 = self.nodes[1]
    ...
    def tagging(self):
        ...
        slme_change_address = n0.getnewaddress()
        asset_change_address = n0.getnewaddress()
        n0.addtagtoaddress(tag, slme_change_address)
        n0.addtagtoaddress(tag, asset_change_address)
        n0.generate(1)
        assert_raises_rpc_error(-8, "bad-txns-null-verifier-address-failed-verification", n0.transfer, asset_name, 100, address, "", 0, slme_change_address, asset_change_address)
        n0.removetagfromaddress(tag, slme_change_address)
        ...
        # special case: make sure transfer fails if the asset change address isn't verified (even if the slme change address is)
        slme_change_address = n0.getnewaddress()
        asset_change_address = n0.getnewaddress()
        assert_raises_rpc_error(-20, "bad-txns-null-verifier-address-failed-verification", n0.transfer, asset_name, 100, address, "", 0, slme_change_address, asset_change_address)
        n0.addtagtoaddress(tag, slme_change_address)
        n0.generate(1)
        assert_raises_rpc_error(-20, "bad-txns-null-verifier-address-failed-verification", n0.transfer, asset_name, 100, address, "", 0, slme_change_address, asset_change_address)
        n0.removetagfromaddress(tag, slme_change_address)
    ...
    def freezing(self):
        ...
        n0.freezeaddress(asset_name, address, slme_change_address)
        n0.generate(1)

        assert_raises_rpc_error(-32600, "freeze-address-when-already-frozen", n0.freezeaddress, asset_name, address, slme_change_address)
        ...
        n0.unfreezeaddress(asset_name, address, slme_change_address)
        n0.generate(1)

        assert_raises_rpc_error(-32600, "unfreeze-address-when-not-frozen", n0.unfreezeaddress, asset_name, address, slme_change_address)
    ...
    def global_freezing(self):
        ...
        n0.freezerestrictedasset(asset_name, slme_change_address)  # Can only freeze once!
        assert_raises_rpc_error(-26, "Freezing transaction already in mempool", n0.freezerestrictedasset, asset_name, slme_change_address)
        n0.generate(1)
        assert_raises_rpc_error(None, "global-freeze-when-already-frozen", n0.freezerestrictedasset, asset_name, slme_change_address)
        ...
        n0.unfreezerestrictedasset(asset_name, slme_change_address)  # Can only un-freeze once!
        assert_raises_rpc_error(-26, "Unfreezing transaction already in mempool", n0.unfreezerestrictedasset, asset_name, slme_change_address)
        n0.generate(1)
        assert_raises_rpc_error(None, "global-unfreeze-when-not-frozen", n0.unfreezerestrictedasset, asset_name, slme_change_address)
