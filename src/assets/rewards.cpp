...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for SLME
    ...
    if (p_rewardSnapshot.strDistributionAsset != "SLME") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is SLME with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using SLME differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "SLME") {
        // Check amount
